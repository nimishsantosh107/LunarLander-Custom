...
class LunarLanderMixed(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        main_action - DISCRETE - MAIN ENGINE: 
            action[0] - Discrete(2)
        side_action - CONTINUOUS - LEFT/RIGHT ENGINE:
            action[1][0] - Box(-1,1) - LEFT:[-1.0,-0.5), OFF:[-0.5,0.5], RIGHT:(0.5,1.0]
        '''
        main_action = action[0]
        side_action = action[1][0]
        
        ########## [TODO] CHECK IF ACTION SPACE CONTAINS PASSED 'action'
        ...
        m_power = 0.0
        if (self.mixed and main_action == 1):
            # Main engine
            m_power = 1.0
            ox = (tip[0] * (4/SCALE + 2 * dispersion[0]) +
                  side[0] * dispersion[1])  # 4 is move a bit downwards, +-2 for randomness
        ...
        s_power = 0.0
        if (self.mixed and np.abs(side_action) > 0.5):
            # Orientation engines
            direction = np.sign(side_action)
            s_power = np.clip(np.abs(side_action), 0.5, 1.0)
            assert s_power >= 0.5 and s_power <= 1.0
